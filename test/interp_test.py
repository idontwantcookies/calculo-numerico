...
from src.interpolation.gregory_newton import GregoryNewton
from src.interpolation.natural_spline import NaturalSpline
from src.interpolation.not_a_knot_spline import NotAKnotSpline
...
class InterpTest(unittest.TestCase):
    ...
    def test_vandermonde_ranks(self):
        x, y = random_points()
        for i in range(1, 6):
            vdm = Vandermonde(x, y, rank=i)
            for x_i, y_i in zip(x, y):
                assert round(vdm(x_i)) == y_i

    def test_lagrange1(self):
        lg = Lagrange(self.x, self.y, rank=1)
        pred = lg(x_est=0.2)
        pred = round(pred, 4)
        assert pred == 1.6408

    def test_lagrange2(self):
        lg = Lagrange(self.x, self.y, rank=2)
        pred = lg(x_est=0.2)
        pred = round(pred, 4)
        assert pred == 1.4141

    def test_newton1(self):
        n = Newton(self.x, self.y, rank=1)
        pred = n(x_est=0.2)
        pred = round(pred, 4)
        assert pred == 1.6408

    def test_newton2(self):
        n = Newton(self.x, self.y, rank=2)
        pred = n(x_est=0.2)
        pred = round(pred, 4)
        assert pred == 1.4141
    ...
    def test_natural_spline(self):
        ...
        spl = NaturalSpline(x, y)
        for i, value in enumerate(x):
            assert round(spl(value)) == y[i]

    def test_natural_spline2(self):
        spl = NaturalSpline(self.x, self.y)
        for x, y in zip(self.x, self.y):
            assert round(spl(x), 4) == y

    def test_extrapolated_spline(self):
        x = np.array([1,2,4,6,7])
        y = np.array([2,4,1,3,3])
        spl = NotAKnotSpline(x, y)
        for i, value in enumerate(x):
            assert round(spl(value)) == y[i]
