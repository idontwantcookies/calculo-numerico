...
def bissection(func, a, b, max_iter=_DEFAULT_MAX_ITER, toler=_DEFAULT_TOLER, debug=False):
    ...
    if Fa > 0:
        a, b, Fa, Fb = b, a, Fb, Fa
    if debug:
        _print_row(_bissection_header, precision)
    for i in range(max_iter):
        err /= 2
        x = (a + b) / 2
        Fx = func(x)
        if debug:
            _print_row([i, a, b, x, Fa, Fb, Fx, err], precision)
        if err < toler:
            break
        if Fx < 0:
            a, Fa = x, Fx
...
def secant(func, a, b, max_iter=_DEFAULT_MAX_ITER, toler=_DEFAULT_TOLER, debug=False):
    precision = -floor(log10(toler))
    if debug:
        _print_row(_linear_header, precision)
    Fa, Fb = func(a), func(b)
    if abs(Fa) < abs(Fb):
        a, b, Fa, Fb = b, a, Fb, Fa
    x, Fx = b, Fb
    for i in range(max_iter):
        ...
        Fx = func(x)
        if debug:
            _print_row([i, a, b, x, Fx, delta_x], precision)
        if abs(delta_x) < toler and abs(Fx) < toler:
            break
        a, b, Fa, Fb = b, x, Fb, Fx
    return x, delta_x
...
def regula_falsi(func, a, b, max_iter=_DEFAULT_MAX_ITER, toler=_DEFAULT_TOLER, debug=False):
    ...
    for i in range(max_iter):
        ...
        if debug:
            _print_row([i, a, b, x, Fx, delta_x], precision)
        if abs(delta_x) < toler and abs(func(x)) < toler:
            break
        if Fx < 0:
            a, Fa = x, Fx
        else:
...
def muller(func, a, c, max_iter=_DEFAULT_MAX_ITER, toler=_DEFAULT_TOLER, debug=False):
    ...
    if debug:
        _print_row(_quad_header, precision)
    for i in range(max_iter):
        if abs(Fx) < toler and abs(delta_x) < toler:
            break
        h1, h2 = c - b, b - a
        r, t = h1 / h2, x
        ...
        Fx = func(x)
        if debug:
            _print_row([i, a, b, c, x, Fx, delta_x], precision)
        if x > b:
            a, Fa = b, Fb
        else:
            c, Fc = b, Fb
...
def wijngaarden_dekker_brent(func, a, b, max_iter=_DEFAULT_MAX_ITER, toler=_DEFAULT_TOLER, debug=False):
    ...
    if debug:
        _print_row(['iter', 'x', 'Fx', 'z'], precision)
    c, Fc = b, Fb
    for i in range(max_iter):
        if Fb * Fc > 0:
            c, Fc, d = a, Fa, b - a
...
def schroder(func, dfunc, x0, m, toler=_DEFAULT_TOLER, max_iter=_DEFAULT_MAX_ITER,
    ...
    for i in range(max_iter):
        ...
        if debug:
            _print_row([i, x, Fx, delta_x], precision)
        if abs(delta_x) < toler and abs(Fx) < toler or abs(DFx) == 0:
            break
    return x, delta_x
